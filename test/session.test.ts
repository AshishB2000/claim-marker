...
describe('session tokens', () => {
  it('round-trips the customer and their policy', () => {
    const token = sign({ sub: 'cust-1', policy: 'POL-9' }, SECRET, 3600, NOW)
  ...
  it('clamps the lifetime to something a session can be', () => {
    const short = verify(sign({ sub: 'a' }, SECRET, 1, NOW), SECRET, NOW)
    const long = verify(sign({ sub: 'a' }, SECRET, 999_999, NOW), SECRET, NOW)
    expect(short!.exp).toBe(NOW / 1000 + MIN_TTL)
    expect(long!.exp).toBe(NOW / 1000 + MAX_TTL)
  })

  it('stops working when it expires', () => {
    const token = sign({ sub: 'cust-1' }, SECRET, 60, NOW)
